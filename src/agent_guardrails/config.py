"""Configuration loading from pyproject.toml [tool.agent-guardrails]."""
...
from agent_guardrails.types import AgentTarget, ProjectType


@dataclass
class Config:
    """Parsed configuration from [tool.agent-guardrails] in pyproject.toml."""
    ...
    def from_pyproject(cls, project_root: Path) -> Config:
        """Load config from pyproject.toml [tool.agent-guardrails] section."""
        ...
        tool_config = data.get("tool", {}).get("agent-guardrails", {})
        if not tool_config:
            return cls()
