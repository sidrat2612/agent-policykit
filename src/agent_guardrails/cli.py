...
def init(ctx: click.Context) -> None:
    ...
    root = Path.cwd()
    console.print("[bold]Initializing agent-guardrails...[/bold]")
    ...
    try:
        ...
        console.print("[yellow]Add [tool.agent-guardrails] section to pyproject.toml[/yellow]")
        console.print("\nExample:")
        console.print('  [tool.agent-guardrails]')
        console.print('  targets = ["copilot", "cursor", "claude-code"]')

    console.print("\n[bold green]Done![/bold green] Run `agent-guardrails generate` to create instruction files.")
...
def generate(ctx: click.Context, target: tuple[str, ...], dry_run: bool, mode: str) -> None:
    ...
    import agent_guardrails.adapters.copilot_repo  # noqa: F401
    import agent_guardrails.adapters.copilot_path  # noqa: F401
...
def update(ctx: click.Context, force: bool, dry_run: bool) -> None:
    ...
    from agent_guardrails.adapters import get_adapter, list_adapters
    from agent_guardrails.analysis.detector import detect_project_context
    from agent_guardrails.core.diff_engine import compute_diff
    from agent_guardrails.core.policy_engine import build_policy_bundle
    from agent_guardrails.core.update_engine import apply_updates
...
def validate(ctx: click.Context) -> None:
    ...
    from agent_guardrails.core.loader import load_governance_packs, load_packs_from_directory, PACKS_DIR
    from agent_guardrails.core.validator import validate_packs
